"""
Go board game -- hardware-accelerated variant.

Board.useful() is offloaded to the `go_useful_accel` block (see hw/).  Here the
block is stood in for by its behavioural model, accel_model.GoUsefulAccel, so
the partition can be proved bit-identical to the baseline on a machine with no
FPGA attached.  On real hardware `ACCEL` becomes a thin MMIO shim; nothing else
...
from accel_model import GoUsefulAccel  # noqa: E402
...
class Board:
    ...
    def __init__(self):
        ...
        # One key bank suffices: computer_move() finishes every useful() call on
        # `board` before `nboard` is constructed.
        ACCEL.load_keys(self.squares)
        self.reset()
    ...
    def useful(self, pos):
        global TIMESTAMP
        TIMESTAMP += 1
        fast, cond, h = ACCEL.useful(pos, self.color)
        if fast:
            return True
        # The exact superko set stays in software: it is unbounded and a false
        # positive would change the game.
        return bool(cond) and h not in self.zobrist.hash_set

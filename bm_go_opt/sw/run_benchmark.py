...
class Square:
    ...
    __slots__ = ('board', 'pos', 'timestamp', 'removestamp', 'zobrist_strings',
                 'neighbours', 'color', 'reference', 'ledges', 'used', 'temp_ledges')
...
#                self.board.white_dead += 1
    ...
    def find(self, update=False):
        node = self
        while node.reference.pos != node.pos:
            node = node.reference
        root = node
        if update:
            node = self
            while node.pos != root.pos:
                nxt = node.reference
                node.reference = root
                node = nxt
        return root
